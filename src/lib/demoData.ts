...
export function getDemoData(mode: 'dm' | 'group', lang: 'en' | 'id'): { metrics: ParsedChatMetrics; insights: GeminiInsights } {
  ...
  if (mode === 'dm') {
    ...
    const metrics: ParsedChatMetrics = {
      ...
      slurCount: { [p1]: 50, [p2]: 20 },
      topSlurs: [{ word: isId ? 'anjing' : 'fuck', count: 35 }, { word: isId ? 'bangsat' : 'shit', count: 20 }],
    ...
    const metrics: ParsedChatMetrics = {
      ...
      slurCount: { [p1]: 420, [p2]: 150, [p3]: 80, [p4]: 5, [p5]: 0 },
      topSlurs: [{ word: isId ? 'anjing' : 'fuck', count: 250 }, { word: isId ? 'bangsat' : 'shit', count: 180 }],
