...
export function generateDemoInsights(metrics: ParsedChatMetrics, language: 'en' | 'id' = 'en', chatMode: 'dm' | 'group' = 'dm'): GeminiInsights {
  ...
  if (chatMode === 'group') {
    ...
    const summaries = [
      `This group is a study in chaos: ${p1} dominates by sending ${p1Pct}% of all ${totalMsg.toLocaleString()} messages, treating the chat like a personal broadcast channel, while others mostly spectate. ${ghostCount > 0 ? `There were ${ghostCount} instances of the chat dying completely for 72 hours before being revived.` : `The group is highly active-rarely quiet for more than 72 hours.`} Together you've built a dynamic that is, at minimum, a consistent circus.`,
      `The dynamic here is heavily skewed: ${p1} drives the bus, sending ${p1Pct}% of the ${totalMsg.toLocaleString()} total messages, leaving everyone else as mere passengers. ${ghostCount > 0 ? `The chat also exhibits ${ghostCount} moments of total radio silence for 72 hours.` : `Impressively, the conversation almost never stops.`} It's a digital ecosystem that defies explanation.`,
      `With ${totalMsg.toLocaleString()} messages in the vault, this is less of a group chat and more of ${p1}'s personal diary (${p1Pct}%), featuring occasional guest appearances. ${ghostCount > 0 ? `You also collectively ghost each other, with ${ghostCount} deep silences of 72 hours.` : `The pacing is relentless, with barely a pause to breathe.`} Truly an unhinged masterpiece of modern communication.`
    ];
    const roasts = [
  ...
  if (language === 'id') {
    const summaries = [
      `Percakapan ini adalah studi kontras: ${bigTexter} mengirimkan ${p1Pct >= p2Pct ? p1Pct : p2Pct}% dari total ${totalMsg.toLocaleString()} pesan, memperlakukan chat ini seperti channel broadcast pribadi, sementara ${quietOne} beroperasi dengan lebih santai. ${quickReplier} adalah pembalas tercepat dengan rata-rata ${quickSpeed} - tipe orang yang ponselnya selalu di tangan. ${ghostCount > 0 ? `Ada ${ghostCount} kali seseorang menghilang selama 72+ jam, entah karena sibuk atau cuma baca doang.` : `Menariknya, tidak ada yang suka ghosting - kecepatan balas cukup bagus.`} Bersama-sama, kalian membangun dinamika yang, paling tidak, konsisten kacau.`,
      `Dinamika percakapan ini didorong oleh ${bigTexter} yang menyumbang ${p1Pct >= p2Pct ? p1Pct : p2Pct}% dari ${totalMsg.toLocaleString()} pesan, meninggalkan ${quietOne} untuk merespon seperlunya saja. Di sisi lain, jari ${quickReplier} bergerak secepat kilat dengan balasan rata-rata ${quickSpeed}. ${ghostCount > 0 ? `Sisi gelapnya, kalian tercatat saling diam 72+ jam sebanyak ${ghostCount} kali.` : `Hebatnya kalian nyaris tidak pernah membiarkan chat menggantung.`} Interaksi ini mendefinisikan apa itu tarik-ulur digital.`,
      `Kalian berdua telah menumpuk ${totalMsg.toLocaleString()} pesan, di mana ${bigTexter} mendominasi ${p1Pct >= p2Pct ? p1Pct : p2Pct}% darinya seolah mengejar kuota kata. ${quietOne} jauh lebih hemat energi. ${quickReplier} pantas dapat piala fast-response karena rekor balas ${quickSpeed}-nya. ${ghostCount > 0 ? `Walau begitu, kebiasaan ngilang berhari-hari sebanyak ${ghostCount} kali cukup membuktikan prioritas ` + formatParticipantPhrase(metrics.participants.length, 'kalian berdua', 'kalian') + `.` : `Konsistensi balas kalian patut diacungi jempol.`} Kombinasi yang unik antara agresif dan pasif.`
    ];
    const roasts = [
