...
function buildPromptPayload(metrics: ParsedChatMetrics): object {
  ...
  return {
    ...
    top_keywords: topKeywords.slice(0, 30),
    era_metrics: eraMetrics,
    // Capped to stay within strict free-tier input-token limits
    sample_excerpts: {
      early: sampleExcerpts.early.slice(0, 6),
      median: sampleExcerpts.median.slice(0, 6),
      late: sampleExcerpts.late.slice(0, 6),
    }
  };
}
...
You MUST respond with valid JSON only, no markdown, no explanation. The JSON must exactly match this schema:
{
  ...
  "topics": ["Array of 3-6 specific topics detected from the keywords and excerpts. E.g. 'weekend plans', 'work complaints', not generic 'chatting'."],
