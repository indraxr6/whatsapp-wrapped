...
export default function ExportMenu() {
  const { setIsSelecting, setIsExporting, isSelecting } = useExportContext();
  const { t } = useLanguage();
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <button
        ...
        className="nb-btn-primary flex items-center gap-1.5 text-[10px] min-[415px]:text-xs py-1 min-[415px]:py-1.5 px-2 min-[415px]:px-3"
        title={t("export.export_all") || "Export Everything"}
      >
        <Download size={14} />
        <span className="hidden sm:inline">{t("export.export_all") || "Export All"}</span>
      </button>
    </div>
  );
}
